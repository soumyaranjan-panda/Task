...
The **partition** divides rows into independent groups. The **frame** then selects a sliding subset _within_ that partition (or the whole partition if there is no `ORDER BY`).
